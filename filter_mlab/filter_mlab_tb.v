...
// ResetInputPort: i_reset_n
// TargetDirectory: C:\Users\oleh_\YandexDisk\Елекроніка\Melexis\Lab4
// Name: filter_mlab
// ResetAssertedLevel: Active-low
// SerialPartition: 9
// TargetLanguage: Verilog
// TestBenchName: filter_mlab_tb
...
//************************************
 ...
 // Block Statements
  ...
  always @(posedge i_clk or negedge i_reset_n) // completed_msg
  begin
    if (~i_reset_n) begin 
       // Nothing to reset.
    end 
  ...
  always @(posedge i_clk or negedge i_reset_n)
  begin
    filter_in_data_log_task(i_clk,i_reset_n,
  ...
  begin // stimuli_filter_in_data_log_filter_in
    if (filter_in_data_log_rdenb == 1) begin
      filter_in <= # i_clk_hold filter_in_data_log_force[filter_in_data_log_addr];
    end
    else begin 
      filter_in <= # i_clk_hold holdData_filter_in;
    end
  ...
  always @(posedge i_clk or negedge i_reset_n)
  begin
    filter_out_task(i_clk,i_reset_n,
  ...
  assign # i_clk_hold filter_out_dataTable = filter_out_expected[filter_out_addr];
...
// ---- Bypass Register ----
  ...
  always @ (posedge i_clk or negedge i_reset_n) // checker_filter_out
  begin
  ...
  always @ (posedge i_clk or negedge i_reset_n) // checkDone_1
  begin
    if (i_reset_n == 0)
      check1_Done <= 0;
